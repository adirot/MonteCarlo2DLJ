...
%a2 = (1-v)/v;
m = 3;
M = MC2DLJoutput(50,t,0.1,1,'random',4,1,...
            'm',m,'ufunc',@(r) -(r.^-m),...
            'angleDependent',true,...
            'angleDependence',@(b,t1,t2)...
                -((6*(v^2-1)*cos(2*(b-t1))-15*v*(1+v)*cos(4*b-2*t1-2*t2)...
                +2*(v^2-1)*(1+3*cos(2*(b-t1)))+...
                (v-2)*(1+v)*cos(2*(t1-t2)))/8),...
            'maxdAng',pi/4,'hcr',true,...
            'dontSaveDists',true,'pressure',false,'dipoleStrength',round(rand(1,50))*10+1);
disp('1');        
M = M.MonteCarlo(100000,10,'logFileInit',['ni' my_num2str(v) '/']);
m = 3;
M = MC2DLJoutput(2,0,0.0001,1,'2par_close',10,(m/12)^(1/(m-12))/2,...
            'pressure',false,'m',m,'ufunc',@(r) -(r.^-m),...
            'angleDependent',true,...
            'angleDependence',@(b,t1,t2) (2*a2 + 6*a2*cos(2*b-2*t1)...
            + 6*a2*cos(2*b-2*t2) + 15*cos(4*b-2*t1-2*t2)+...
            (1+2*a2)*cos(2*t1-2*t2)),...
            'maxdAng',pi/4,'hcr',true,...
            'fileNameInit','angdep/','dontSaveDists',true,'pressure',false);
